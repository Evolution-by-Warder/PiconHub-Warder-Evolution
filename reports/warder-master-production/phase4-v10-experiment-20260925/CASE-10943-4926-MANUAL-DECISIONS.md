...
|---|---|---|---|---|---|
| #10943 | Rebuild from original transparent source; upper `S`, `H`, and `W` black; preserve the red `O` button and complete silver bezel | `b9a2e1468c5ed7a235ea784c03ed343507f6056d77e5c364fe4731a7bcf11fed` | `8ed781d99e90f53430d0c3c65812d58f47dab563dab0c6d0c5cf7e061a660e9f` | Correction requested; awaiting visual review | REVIEW |
...
Both edits were regenerated from each case's own original transparent source using the frozen V9 fit/render and exact WHITE MASTER; the reconstructed baseline matched the previous/current output. For #10943, the rejected candidate was replaced by a fresh render from the original transparent source and immutable WHITE MASTER. Only upper `S`, `H`, and `W` lettering is manually recolored; the red button, full silver `O` bezel, source alpha, and source file remain unchanged. The manual source mask covers 6,273 pixels; the corrected composite differs from the rejected candidate in 1,140 output pixels. The lower `RADYO` output is unchanged. For #4926, 302 source pixels in the last `L` only were recolored (278 output pixels differ); source alpha and chromatic pixels were preserved. #7564 shares #4926's transparent-source SHA256 but was not changed by this decision.
